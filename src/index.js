...
async function resolveWithCobalt(env, { url, quality }) {
  ...
  if (data.status === "error") {
    return { error: data.text || data.error?.code || "extraction_failed" };
  }
...
async function handleDownload(request, env) {
  ...
  if (result.error) {
    return json({ error: humanizeError(result.error) }, 502);
  }
...
function humanizeError(code) {
  const map = {
    ...
    "content.video.unavailable": "That video isn't available.",
